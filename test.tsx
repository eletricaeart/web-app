...
export default function ModeloVisualStudio() {
  ...
  const addOpening = (type: 'door' | 'window' | 'opening') => {
    ...
    let posY: number | undefined = 0; // padrão para portas e vãos

    // Se for janela, tentar alinhar topo com a porta existente
    if (type === 'window') {
      const doorOpening = selectedRoom.openings.find((o) => o.type === 'door');
      if (doorOpening) {
        // altura da porta = doorOpening.height
        // queremos que o topo da janela fique no mesmo nível do topo da porta
        // posY da janela = altura_da_porta - altura_da_janela (mas a altura da janela ainda não foi definida, será 0)
        // Como a altura da janela pode ser definida depois, deixamos posY = altura_da_porta (assumindo que a janela terá a mesma altura da porta?)
        // Melhor: posY = altura_da_porta (topo da porta). O usuário ajustará a altura depois.
        posY = doorOpening.height; // isso coloca o topo da janela na mesma altura do topo da porta
      } else {
        posY = 0; // se não houver porta, janela no chão (ou peitoril 0)
      }
    }
                                ...
                                <div className="p-3 pt-0 border-t border-slate-100 space-y-2">
                                  <div className="flex flex-wrap items-center gap-2 text-xs">
                                    ...
                                    <div className="flex items-center gap-1">
                                      ...
                                      <input
                                        type="text" // mudamos para text para evitar problemas com número
                                        step="0.1"
                                        min="0"
                                        value={
                                          op.posY !== undefined &&
                                        ...
                                        onChange={(e) => {
                                          ...
                                          // permite apagar completamente (deixa vazio) ou converte para número
                                          if (val === '') {
                                            updateSelectedRoom({
                                              openings:
                                                selectedRoom.openings.map(
                                                  (o) =>
                                                    o.id === op.id
                                                      ? {
                                                          ...o,
    ...
    <div className="w-full overflow-auto">
      ...
      >
        ...
        {openings.map((op: any) => {
          ...
          const posY = op.posY ?? 0; // distância do chão
          const topY = height - posY - op.height; // se a base está em posY, o topo fica mais acima
          // A altura do vão é op.height
          // A base do vão no SVG é topY + op.height = height - posY
          const baseY = topY + op.height;
          const y = height - posY - op.height; // coordenada no SVG (topo)
          const x = posX;
