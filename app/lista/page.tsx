...
function ListContent() {
  ...
  useEffect(() => {
    ...
    if (d) {
      try {
        ...
        // Carrega o estado de marcação do cliente localmente para não perder o progresso no celular
  ...
  const toggleItem = (itemId: string) => {
    ...
    localStorage.setItem(
      `@ea:public-list-checked:${list.id}`,
  ...
  return (
    <>
      ...
      >
        ...
        {/* EACard com degradê azul elegante */}
        <div
          style={{
            display: 'grid',
          ...
          >
            ...
            <div>
              ...
              <strong style={{ color: '#00559c', fontSize: '12px' }}>
                {checked} de {total} itens ({progress}%)
              </strong>
            </div>
          </div>
        </div>
          ...
          <tbody>
            {list.items.map((item, idx) => {
              ...
              if (item.unitPrice) {
                ...
                const q =
                  parseFloat((item.quantity || '1').replace(/[^\d.]/g, '')) ||
                  1;
                if (!isNaN(p) && p > 0) {
                  ...
                  <td style={{ padding: '7px 8px', textAlign: 'center' }}>
                    <div
                      style={{
                        width: '14px',
                        height: '14px',
                        border: `1.5px solid ${item.checked ? '#00559c' : '#94a3b8'}`,
                        borderRadius: '3px',
                        margin: '0 auto',
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        backgroundColor: item.checked ? '#00559c' : '#ffffff',
                        color: '#ffffff',
                        fontSize: '9px',
                        fontWeight: 'bold',
                      }}
        ...
        >
          ...
          <strong style={{ color: '#00559c' }}>
            {list.items.length} itens listados
          </strong>
        </div>
      </div>

      {/* 2. VISUALIZAÇÃO INTERATIVA EM TELA (Mobile & Desktop) */}
      <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#13151A] text-slate-800 dark:text-slate-100 font-sans pb-28 selection:bg-[#00559c]/20 print:hidden">
        {/* Premium Header */}
              ...
              <div className="min-w-0">
                ...
                {(list.clientName || list.orcamentoName) && (
                  <div className="flex flex-wrap items-center gap-2 mt-2.5">
                    {list.clientName && (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[#edf4fa] dark:bg-[#00559c]/20 text-xs font-semibold text-[#00559c] dark:text-[#58a6ff] border border-[#00559c]/20">
                        <Users size={14} />
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-4 flex items-center justify-between gap-2">
          <button
            onClick={handlePrint}
            className="flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-white dark:bg-[#1C1F26] border border-slate-200 dark:border-slate-800 hover:border-[#00559c]/50 text-slate-700 dark:text-slate-200 font-bold text-xs active:scale-[0.98] transition-all shadow-sm"
          ...
          <button
            onClick={handleShare}
            className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-[#edf4fa] dark:bg-[#00559c]/20 border border-[#00559c]/30 hover:bg-[#00559c]/15 text-[#00559c] dark:text-[#58a6ff] font-bold text-xs active:scale-[0.98] transition-all shadow-sm"
          ...
          <AnimatePresence>
            {isComplete && (
              <motion.div
                ...
                className="bg-gradient-to-r from-[#00559c] to-[#0077d4] rounded-3xl p-5 mb-6 text-white shadow-lg shadow-[#00559c]/25 flex items-center justify-between"
              >
                <div>
                  <h2 className="text-lg font-black mb-0.5">Tudo Pronto! 🎉</h2>
                  <p className="text-blue-100 font-medium text-xs">
                    Você marcou todos os itens desta lista de compras.
                  </p>
                </div>
                  ...
                  .map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {item.checked && (
                          <div className="absolute inset-0 border-2 border-[#00559c]/20 rounded-2xl pointer-events-none" />
                        )}
                      </motion.div>
                    );
                  })
