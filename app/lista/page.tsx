...
function parseCurrency(priceStr?: string): number {
  ...
  const p = parseFloat(priceStr.replace(/\./g, '').replace(',', '.').replace(/[^\d.]/g, ''));
  return isNaN(p) ? 0 : p;
}
...
function ListContent() {
  ...
  const saveEdits = (newEdits: Record<string, ClientItemEdit>) => {
    ...
    if (list?.id) {
      localStorage.setItem(`@ea:public-list-edits:${list.id}`, JSON.stringify(newEdits));
    }
  };
  ...
  const openEditModal = (item: MaterialItem) => {
    ...
    const req = parseQuantity(item.quantity);
    setEditingItem(item);
    setEditQty(edit?.purchasedQty !== undefined ? edit.purchasedQty : req.number);
    setEditPrice(edit?.actualUnitPrice !== undefined ? edit.actualUnitPrice : (item.unitPrice || ''));
  };
  ...
  const handleSendWhatsappSummary = () => {
    ...
    list.items.forEach(it => {
      ...
      const purchased = edit?.purchasedQty !== undefined ? edit.purchasedQty : (isChecked ? req.number : 0);
      ...
      if (isChecked && purchased > 0) {
        ...
        if (purchased < req.number) {
          ...
          itensParciais.push(`• *${it.name}*: comprou ${purchased} ${req.unit} (faltam ${falta} ${req.unit})`);
        } else {
          itensCompletos++;
        }
      } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] dark:bg-[#13151A] p-4 text-center">
        <div className="bg-white dark:bg-[#1C1F26] p-8 rounded-3xl shadow-sm border border-red-100 dark:border-red-900/30 max-w-sm w-full">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">O link fornecido pode estar quebrado, incompleto ou corrompido.</p>
        </div>
      </div>
    );
  }
  ...
  if (!list) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#F8FAFC] dark:bg-[#13151A]">
        ...
        <p className="mt-4 text-slate-500 dark:text-slate-400 font-medium animate-pulse">Carregando materiais...</p>
      </div>
    );
  }
  ...
  const progress = totalItemsCount === 0 ? 0 : Math.round((fullyPurchasedCount / totalItemsCount) * 100);
  const isComplete = totalItemsCount > 0 && fullyPurchasedCount === totalItemsCount;
  ...
  return (
    <>
      ...
      <div className="hidden print:block w-full text-slate-900 bg-white" id="public-print-view">
        ...
        <div 
          style={{
            ...
            backgroundImage: "linear-gradient(135deg, rgba(5, 22, 42, 0.94) 0%, rgba(0, 51, 102, 0.90) 50%, rgba(0, 85, 156, 0.88) 100%), url('https://res.cloudinary.com/dyycxyttb/image/upload/v1772753715/bg3_jwcwgx.png')",
        ...
        {/* Card Informativo */}
        <div style={{ border: '1px solid rgba(0, 85, 156, 0.2)', borderRadius: '8px', padding: '10px 14px', marginBottom: '12px', backgroundColor: '#ffffff' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderBottom: '2px solid #00559c', paddingBottom: '6px', marginBottom: '8px' }}>
            <div>
              <span style={{ fontSize: '9.5px', fontWeight: 700, color: '#00559c', textTransform: 'uppercase', letterSpacing: '0.1em', display: 'block' }}>Documento de Quantitativos & Compras</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr 1.2fr', gap: '10px' }}>
            ...
            <div>
              <span style={{ color: '#64748b', fontSize: '9.5px', textTransform: 'uppercase', fontWeight: 600, display: 'block' }}>Status da Compra</span>
              <strong style={{ color: '#00559c', fontSize: '12px' }}>{fullyPurchasedCount}/{totalItemsCount} ({progress}%)</strong>
            </div>
            <div>
              <span style={{ color: '#64748b', fontSize: '9.5px', textTransform: 'uppercase', fontWeight: 600, display: 'block' }}>Total Gasto Comprado</span>
              <strong style={{ color: '#00559c', fontSize: '13px' }}>R$ {formatBRL(totalGastoCliente)}</strong>
            </div>
          </div>
        </div>
        ...
        <table className="print-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px', marginTop: '8px' }}>
          <thead>
            <tr style={{ backgroundColor: '#00559c', color: '#ffffff' }}>
              <th style={{ width: '32px', padding: '8px 4px', textAlign: 'center', fontWeight: 800, borderRight: '1px solid rgba(255,255,255,0.2)' }}>#</th>
              <th style={{ padding: '8px 10px', textAlign: 'left', fontWeight: 800, borderRight: '1px solid rgba(255,255,255,0.2)' }}>Material / Descrição</th>
              <th style={{ width: '85px', padding: '8px', textAlign: 'center', fontWeight: 800, borderRight: '1px solid rgba(255,255,255,0.2)' }}>Qtd. Necessária</th>
              <th style={{ width: '105px', padding: '8px', textAlign: 'center', fontWeight: 800, borderRight: '1px solid rgba(255,255,255,0.2)' }}>Qtd. Comprada</th>
              <th style={{ width: '85px', padding: '8px 10px', textAlign: 'right', fontWeight: 800, borderRight: '1px solid rgba(255,255,255,0.2)' }}>Preço Unit.</th>
              <th style={{ width: '90px', padding: '8px 10px', textAlign: 'right', fontWeight: 800, borderRight: '1px solid rgba(255,255,255,0.2)' }}>Total Item</th>
              <th style={{ width: '45px', padding: '8px', textAlign: 'center', fontWeight: 800 }}>Status</th>
            </tr>
          </thead>
          <tbody>
            {list.items.map((item, idx) => {
              ...
              const isChecked = edit?.checked ?? false;
              const purchased = edit?.purchasedQty !== undefined ? edit.purchasedQty : (isChecked ? req.number : 0);
              const unitP = parseCurrency(edit?.actualUnitPrice || item.unitPrice);
              const totalItem = purchased > 0 ? (purchased * unitP) : (req.number * unitP);
              ...
              return (
                ...
                >
                  <td style={{ padding: '7px 4px', textAlign: 'center', fontWeight: 700, color: '#00559c', borderRight: '1px solid rgba(0,85,156,0.08)' }}>
                    {idx + 1}
                  </td>
                  <td style={{ padding: '7px 10px', borderRight: '1px solid rgba(0,85,156,0.08)' }}>
                    <div style={{ fontWeight: 700, color: '#0f172a' }}>{item.name}</div>
                    {item.description && <div style={{ fontSize: '10px', color: '#64748b', marginTop: '1px' }}>{item.description}</div>}
                  </td>
                  <td style={{ padding: '7px 8px', textAlign: 'center', fontWeight: 700, color: '#0f172a', borderRight: '1px solid rgba(0,85,156,0.08)' }}>
                    {req.number} {req.unit}
                  </td>
                  <td style={{ padding: '7px 8px', textAlign: 'center', borderRight: '1px solid rgba(0,85,156,0.08)' }}>
                    {isChecked ? (
                      purchased < req.number ? (
                        <div style={{ color: '#b45309', fontWeight: 700 }}>
                          {purchased} {req.unit} <span style={{ fontSize: '9.5px', color: '#d97706' }}>(Falta {req.number - purchased})</span>
                        </div>
                      ) : (
        ...
        <div style={{ marginTop: '12px', padding: '10px 14px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '6px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '11px', color: '#64748b' }}>
          <span>Elétrica & Art &bull; Soluções em Engenharia e Instalações Elétricas</span>
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
            <span>Estimado: <strong>R$ {formatBRL(totalEstimadoLista)}</strong></span>
            <span style={{ color: '#00559c', fontSize: '12px' }}>Total Gasto Real: <strong>R$ {formatBRL(totalGastoCliente)}</strong></span>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#13151A] text-slate-800 dark:text-slate-100 font-sans pb-32 selection:bg-[#00559c]/20 print:hidden">
             ...
             <div className="flex items-start justify-between gap-4">
                ...
                <div className="text-right shrink-0 flex flex-col items-end">
                  <span className="block text-3xl sm:text-4xl font-black text-slate-800 dark:text-white tabular-nums tracking-tighter leading-none">
                    {progress}<span className="text-lg text-[#00559c] dark:text-[#58a6ff]">%</span>
                  </span>
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-4">
          <div className="bg-white dark:bg-[#1C1F26] rounded-2xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Total Gasto Comprado</span>
            ...
            <div className="text-right shrink-0">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Total Estimado</span>
        ...
        {/* Botões Rápidos de Ação: Imprimir / PDF & Enviar WhatsApp */}
        <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-3 flex items-center justify-between gap-2">
          ...
          >
            <Printer size={16} weight="bold" className="text-[#00559c] dark:text-[#58a6ff]" />
          ...
          >
            <WhatsappLogo size={17} weight="fill" className="text-emerald-600 dark:text-emerald-400" />
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-3">
          <p className="text-[11.5px] text-slate-500 dark:text-slate-400 flex items-center gap-1.5 bg-slate-100/70 dark:bg-slate-800/50 py-1.5 px-3 rounded-lg">
            ...
            <span>Toque em qualquer material para ajustar a <strong>quantidade comprada</strong> e o <strong>preço pago</strong>.</span>
          </p>
        </div>
            ...
            {isComplete && (
              ...
              >
                <div>
                  ...
                  <p className="text-blue-100 font-medium text-xs">Todos os itens solicitados foram adquiridos.</p>
                </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <button 
                        ...
                        title={isChecked ? 'Desmarcar item' : 'Marcar como comprado'}
                      >
                        <CheckCircle size={18} weight="bold" className={isChecked ? 'opacity-100' : 'opacity-0'} />
                      </button>
                      
                      {/* Conteúdo do Item */}
                      <div className="flex-1 min-w-0 flex flex-col">
                        <div className="flex items-center gap-2 flex-wrap">
                          {/* Badge de Quantidade Inteligente */}
                          {isChecked ? (
                            isPartial ? (
                              <span className="px-2 py-0.5 text-xs font-bold rounded-md shrink-0 bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-800">
                                Comprou {purchased} de {req.number} {req.unit} (Falta {req.number - purchased})
                              </span>
                            ) : (
                            ...
                            <span className="px-2 py-0.5 text-xs font-bold rounded-md shrink-0 bg-[#edf4fa] dark:bg-[#00559c]/20 text-[#00559c] dark:text-[#58a6ff] border border-[#00559c]/20">
                              {req.number} {req.unit} necessário
                            </span>
                          )}

                          <span className={`font-semibold text-[14.5px] truncate transition-all duration-300 ${
                            isChecked && !isPartial ? 'text-slate-400 dark:text-slate-500 line-through' : 'text-slate-700 dark:text-slate-100'
                          }`}>
                            {item.name}
                          </span>
                        </div>
                          ...
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            {unitP > 0 && (
                              <span className="text-slate-500 dark:text-slate-400">
                                Unit: <strong className="text-slate-700 dark:text-slate-200">R$ {formatBRL(unitP)}</strong>
                              </span>
                            )}
                          </div>
        ...
        <div className="fixed bottom-6 right-6 flex items-center gap-3 z-40">
          <motion.button
            ...
            className="w-12 h-12 bg-white dark:bg-[#1E222B] text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 rounded-full flex items-center justify-center shadow-xl active:scale-95 transition-transform hover:border-[#00559c]"
          >
          ...
          <motion.button
            ...
            className="w-12 h-12 bg-emerald-600 hover:bg-emerald-500 text-white rounded-full flex items-center justify-center shadow-xl active:scale-95 transition-transform shadow-emerald-600/30"
          >
          ...
          <motion.button
            ...
            onClick={handleShare}
            title="Compartilhar Link"
            className="w-14 h-14 bg-[#00559c] hover:bg-[#004785] text-white rounded-full flex items-center justify-center shadow-2xl active:scale-95 transition-transform shadow-[#00559c]/40"
          >
                ...
                >
                  ...
                  <div className="py-5 space-y-5">
                    ...
                    <div className="bg-[#edf4fa] dark:bg-[#00559c]/20 p-3.5 rounded-2xl border border-[#00559c]/20 flex items-center justify-between">
                      ...
                      <span className="text-xs px-2.5 py-1 rounded-full bg-white dark:bg-[#1C1F26] text-slate-600 dark:text-slate-300 font-semibold shadow-xs">
                    ...
                    <div>
                      ...
                      {diff > 0 && editQty > 0 && (
                        <div className="mt-2.5 flex items-center gap-1.5 p-2 rounded-lg bg-amber-50 dark:bg-amber-950/30 text-amber-800 dark:text-amber-300 text-xs font-medium border border-amber-200 dark:border-amber-900/40">
                          <Warning size={15} weight="bold" className="shrink-0 text-amber-600" />
                          <span>Falta comprar <strong>{diff} {req.unit}</strong> para completar a solicitação do projeto.</span>
                        </div>
                      )}
                      {editQty === req.number && (
                        <div className="mt-2.5 flex items-center gap-1.5 p-2 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 text-emerald-800 dark:text-emerald-300 text-xs font-medium border border-emerald-200 dark:border-emerald-900/40">
                          <CheckCircle size={15} weight="bold" className="shrink-0 text-emerald-600" />
                          <span>Quantidade completa atendida!</span>
                        </div>
                      )}
                      {diff < 0 && (
                        <div className="mt-2.5 flex items-center gap-1.5 p-2 rounded-lg bg-blue-50 dark:bg-blue-950/30 text-blue-800 dark:text-blue-300 text-xs font-medium border border-blue-200 dark:border-blue-900/40">
                          <span>Você comprou <strong>{Math.abs(diff)} {req.unit} a mais</strong> do que o especificado.</span>
                        </div>
                      )}
                    </div>

                    {/* Preço Unitário Pago Real */}
                    <div>
                      <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white font-bold text-base focus:outline-none focus:border-[#00559c]"
                        />
                      </div>
                    </div>
                    ...
                    <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-900/80 border border-slate-100 dark:border-slate-800 flex justify-between items-center">
                      <div>
                        <span className="text-[10.5px] font-bold text-slate-400 uppercase tracking-wider block">Total Deste Item</span>
